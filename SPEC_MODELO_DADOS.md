...
- [x] 5.4a **Comparativo R$/m² por unidade × mercado (ZONA 3, puxado à frente por ter dado real; publicado):** para cada unidade, `valorAluguel ÷ metragens.construidaM2` (pé-direito NÃO entra) comparado à mediana de mercado do tipo (mesma conta dos comparáveis). 3 estados confirmados no Gate 1 — **plota** (aluguel+área+mediana), **alerta_falta** (aluguel sem área), **ausente** (sem aluguel) — mais **sem_mercado** (aluguel+área mas tipo sem comparável, ex. terreno/oficina): mostra o número, barra neutra, sem comparar. Tipo de comparação por unidade via **Opção B — mapa manual em Config** (`imobiliario_unidadeTipoComparacao`, `{unidadeId: TipoAtivo}`; ausente = usa o tipo do ativo pai). Escolhida sobre campo em `Unidade` por serem poucos casos; reversível, sem migração. Painel na aba Imobiliário → Dados. Só leitura + Config. Backup round-trip lossless (áreas por unidade + mapa de tipo preservados).
- [ ] 5.4 ZONA 3 — reuso dos comparativos (R$/m², funil de hipóteses) no dashboard (levar o comparativo por unidade e o funil para o frontend).
