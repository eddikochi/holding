...
function importarHubV1(o: Record<string, unknown>): ResultadoImport {
  ...
  function obterHipotese(enunciado: string): Hipotese {
    ...
    if (!h) {
      ...
      h = {
        ...
        enunciado: enunciado.trim(), pilares: ['logistico'],
  ...
  for (const raw of players) {
    ...
    if (stakeholder.dorOportunidade) {
      ...
      evidencias.push({
        ...
        vinculos: stakeholder.hipoteseId ? [{ hipoteseId: stakeholder.hipoteseId, efeito: 'sustenta' }] : [],
        confianca: 'media',
  ...
  for (const raw of entrevistas) {
    ...
    evidencias.push({
      ...
      stakeholderId,
      vinculos: (() => { const hid = stakeholderId ? playerHip.get(stakeholderId) : undefined; return hid ? [{ hipoteseId: hid, efeito: 'sustenta' as const }] : []; })(),
      confianca: 'media',
      origem: metaOrigem('campo_hub_v1', {}),
    });
  }
